...
description: >
  Use when a Miadi/Hermes design branch needs academic foundations: MECE field
  decomposition, canonical concepts, citations, engineering implications,
  chronicle seeds, visual seeds, and machine-readable source ledgers under a
  root-level foundations/ folder. Forked from deep-research for Storyweaver / 
  refactoring-portal research that must satisfy academic, engineering, and
  narrative audiences.
version: 0.1.0
---
...
Multi-agent research orchestrator for creating durable `foundations/<academic-field>/` artifacts. Decomposes a design question into 3-6 academic/professional fields using MECE principles, researches each field, runs gap analysis, then synthesizes a foundations map, chronicle seeds, visual seeds, and a source ledger for future Miadi/Hermes work.

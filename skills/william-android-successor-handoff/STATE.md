...
## 2026-08-31 — Episodes 343–345 and first Review-to-Wheel relation

- **Found:** the event-bus witness was the duplicate Episode 098; the home-and-aspiration vessel is now the sole 098. Miadi Review `6394c566-081e-4f89-bace-3758066534a1` grounds graph engineering but does not itself prescribe vocabulary-first; that is Episode 344's declared use. The Review API token was unset, so only the refreshed tracked cache and public HTTP 200 page were read.
- **Changed:** pulled `jgwill/Miadi` `main` to `9aa15d79`, upgraded global `@miadi/inquiry-weave` to `0.8.3`, and linked Pi-global `~/.pi/agent/skills/miadi-review` to the repository skill. Moved the duplicate witness with `git mv` into Episode 343; created Episodes 344 and 345; registered one reusable Review node, a manifest-missing legacy Episode 094 node, and six task relations. Chronicle commit `a860444` is pushed on `origin/main`.
- **Verified:** Episodes 343–345 each have registered receipts and direct Wheel node HTTP 200. The Wheel edge list contains Episode 343→094, 344→343/340, 345→344, plus `uses-to-ground-vocabulary` and `documents-review-relation-practice` edges to the Review node. Episode 345 carries the repeatable practice and machine receipt.
- **Failed or deferred:** no authenticated Review API query ran; cached version 7 is the version observed, not a claim about a newer unseen service version. Episode 094 remains manifest-less and its Wheel node says `status: manifest-missing`. Forgewright was deliberately outside this experiment.
- **Next safe move:** use Episode 345's practice to design an idempotent Review-to-Wheel helper and UI projection that render review URL/version, episode-specific intended contribution, and obligations without hand-shaped API payloads.

## 2026-08-31 — Manual Review import and Episode 345 session voice

- **Found:** Matt Pocock's prepared “Software Fundamentals in the Age of AI” review explicitly grounds shared design intent, ubiquitous language, TDD feedback, deep modules, gray boxes, and strategic human oversight. Medicine Wheel app `0.6.3` exposes 27 ordered workspace packages whose names provide a second measured vocabulary source.
- **Changed:** imported the supplied Markdown directly as Review `1e1ba57a-9e30-41ea-8126-5735c3f344bd`; version 1 preserves the initial import and version 2 normalizes only source-line whitespace. Registered its reusable Wheel node and two intention-bearing episode edges. Miadi commit `f21bad96` adds safe `${MIADI_REVIEW_ENV_FILE:-$HOME/.env}` credential discovery, smart-quote support, portable normalized caches, and the refreshed ten-review cache. Chronicle commit `31ba470` adds the package crosswalk and import/graph receipts; `c069f7b` adds the complete 28-turn session dialogue and companion report under Episode 345 ceremony `279972a5-f512-4d81-b74a-91a6bf651531`. All three commits are pushed.
- **Verified:** authenticated retrieval reports `manual-import` version 2 and is byte-identical to the Chronicle source at SHA-256 `bc05ee04789f43a52c2439d7cce4ca16389b3f6bb036634b8ceb3f486ff09b2f`; public review and Wheel node return HTTP 200; both new exact edge types exist; 19 Review-client tests pass. Every ceremony dialogue turn and report aspect is under 55 words, and Chronicle `main` has zero unpushed commits.
- **Failed or deferred:** Episode 094 remains explicitly manifest-missing. The edge API still lacks a first-class prose-intention field and permits only one edge per ordered node pair. The Episode Recorder root returned HTTP 200, but shell inspection did not prove the selected ceremony text was rendered; the committed Chronicle files are the durable authority.

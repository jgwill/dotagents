# Voie 2 — généraliser l'outillage à tous les ateliers

**Tu portes le moteur.** `jamai-watch` et `jamai-on-drop` fonctionnent, mais ils
sont nommés et câblés pour un seul atelier.

## État désiré

Un outil paramétré — `atelier-watch <nom>` et son crochet — qui sert les six
ateliers sans copier-coller, avec un état par veille et par atelier.

## Étapes

1. Lis les cinq scripts et sépare ligne à ligne ce qui est **propre à jamai**
   de ce qui est **général**.
2. Tranche la forme, et nomme-la comme un choix : veille paramétrée, crochet
   par atelier, ou crochet commun à règles. La voie 3 te donne les unités.
3. Implémente la veille paramétrée **avec un dossier d'état par veille**.
4. Implémente le crochet générique **en gardant le discriminant du grave**.
5. Vérifie explicitement qu'aucun repère partagé et aucun seuil de médium n'a
   survécu dans le générique.

## Tension

Entre un outil éprouvé sur un cas et six cas dont quatre n'ont pas encore de
nom d'unité. Ne bloque pas sur les quatre : livre le paramétrable, laisse la
table des unités en donnée d'entrée.

## Ce qui appartient à Jerry

Où vivent les outils généralisés — dans la compétence `jamai-morning`, dans une
nouvelle compétence, ou dans le dépôt `gmtermux` ? Non tranché. Propose, ne
décide pas.

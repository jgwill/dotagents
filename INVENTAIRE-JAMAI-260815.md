...
**Consentement (contrat I3 du dépôt).** Ce fichier nomme l'*existence* et le
*chemin* de matières intimes (enregistrements, mémos vocaux) sans en publier le
contenu — lecture retenue après relecture 🌿 : ici, nommer est la condition de
la protection, car c'est l'inventaire lui-même qui rend visible que la matière
jouée et chantée est la seule chose que rien ne protège. Une citation nommant
une personne tierce a été élidée le jour même ; le scrub de l'historique, s'il
est voulu, est un geste humain et reste ouvert.
...
| | |
|---|---|
| emplacements distincts | **57** |
| paires de doublons vérifiées au sha256 | **41** — 38 identiques, **3 divergentes** |
| fichiers en **autorité hors dépôt** (exemplaire unique, hors de tout git) | **536** — recompté au contrôle adverse |
...
f3f431f (op-015 — message de commit élidé : il nomme une personne tierce)
...
**Aucune branche distante ne contient HEAD.** Les opus 001 → 015 — 345 fichiers
d'arbre de travail (le premier relevé disait 732 : il comptait le `.git`),
...
### ✔︎ revérifié — trois exécutables hors de tout dépôt
...
ces trois-là sont l'exception. *(Le contrôle adverse a trouvé des instantanés
octet-identiques de deux d'entre eux dans `~/.claude/file-history/` — un cache de
session, pas un dépôt ; `jamai-voir-fork` est, lui, réellement unique.)* `jamai-cast-visual` et `jamai-say-kitchen` sont
exactement les outils de la diffusion cuisine.

### Le reste de l'autorité hors dépôt — 536 fichiers (somme recomptée après contrôle)

| emplacement | fichiers | nature |
|---|---|---|
| `~/Recordings-jamai/` | 188 (1,6 Go) | la matière première |
| `~/.assembly-gemini/` (jamai) | 219 | état + sorties de l'agent Zulip, gelés en mai 2026 |
...
| `~/.local/state/episode-voice/` (jamai) | 16 | état de la veille (l'état vivant bouge encore aujourd'hui) |
...
| `~/Downloads/ARCH251217/…` | 4 | archives juin 2025 + 2 gerbers KiCad du sigil |
| `~/.config/systemd/user/jamai-watch.service` | **1** | **le câblage de la veille, exemplaire unique** |
| `~/.claude/agents/jamai.md` | 1 | |
| `~/.local/state/pixel-recorder/jamai-8828.log` | 1 | journal du recorder (§F, oublié de la première somme) |
| `~/salix/simexp/{20260713,20260718}/*_Jamai.md` | 2 | **lieu nº 57, manqué au premier relevé** — `salix/simexp` n'est pas un dépôt |
...
| `~/compositions-jamai` | 212 non suivis, 3 modifiés, **56 commits non poussés** (contre un `origin/main` fetché le 2026-08-02 — le retard distant réel exige un fetch) | voir ci-dessus |
| `~/.hermes` (jamai) | **7 non suivis** | les 3 skills JamAI de Hermes (3 SKILL.md), 3 références, 1 surface-check — **6 sans aucune copie ailleurs** |

---

## Table principale — 57 emplacements
...
| 20 | `plugins/assembly-voice/agents/jamai.json` | config (768 o) | copie identique ×5 vérifiées — et **exception du §D** : ce chemin est un dépôt git imbriqué où le fichier est suivi et propre |
...
| # | chemin | contenu | état |
|---|---|---|---|
| 22 | `~/compositions-jamai` | **1,5 Go**, 345 fichiers d'arbre de travail, opus 001→015 | ⚠️ `Gerico1007/assembly-jamai` — HEAD sur **aucune** branche distante |
...
| 29 | `~/tide-score-visibility-test/jamai_creative_riff.abc` | riff de test | **symlink** → `mcp-musescore-gmusic/sessionABC/…` — **suivi et propre là-bas** ; retiré de la table des 536 (verdict du contrôle adverse : le premier relevé le croyait orphelin) |
| 30 | `/tmp/jamai-transcriptions-260808/` | 5 JSON de transcription | ❌ **dans `/tmp`** |
| 31 | `~/.assembly-gemini/` (jamai) | 219 fichiers, agent Zulip | ❌ hors dépôt |
...
| 34 | `~/.local/state/episode-voice/*jamai*` | 16 fichiers d'état de veille |
...
| # | chemin | contenu |
|---|---|---|
| 55 | `~/Downloads/ARCH251217/…` | `jamai-portal.tsx`, `JamaiComposer.zip`, 2 gerbers KiCad du sigil (2025) |
...
| `jamai.json` ×5 assembly-voice (`96c5562d…`) — 5 copies vérifiées dans le périmètre accessible | identiques |
...
| **`jamai-clip` : `jamai-montage/scripts/jamai-clip` (`f60fef32…`, 188 l.) ↔ `jamai-morning/scripts/jamai-clip.py` (`ff66351a…`, 155 l.)** | ⚠️ **divergents — deux outils de clip dans deux skills du même dépôt** |
...
| `~/.assembly-gemini/state/team-agents/jamai/` | 164 fichiers gelés en mai 2026, plus lus (219 pour tout `.assembly-gemini` jamai) |
